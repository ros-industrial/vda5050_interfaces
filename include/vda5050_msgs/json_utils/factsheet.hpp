...
void from_json(const nlohmann::json& j, AgvAction& msg)
{
    ...
    {
        msg.factsheet_action_parameters = j.at("factsheetActionParameters").get<std::vector<FactsheetActionParameter>>();
    }

    if (j.contains("resultDescription"))
    {
        msg.result_description.push_back(j.at("resultDescription").get<std::string>());
    }
...
void from_json(const nlohmann::json& j, LoadSet& msg)
{
    ...
    {
        msg.bounding_box_reference.push_back(j.at("boundingBoxReference").get<BoundingBoxReference>());
    }
    ...
    {
        msg.min_load_handling_height.push_back(j.at("minLoadHandlingHeight").get<double>());
    }

    if (j.contains("maxLoadHandlingHeight"))
    {
        msg.max_load_handling_height.push_back(j.at("maxLoadHandlingHeight").get<double>());
    }

    if (j.contains("minLoadHandlingDepth"))
    {
        msg.min_load_handling_depth.push_back(j.at("minLoadHandlingDepth").get<double>());
    }

    if (j.contains("maxLoadHandlingDepth"))
    {
        msg.max_load_handling_depth.push_back(j.at("maxLoadHandlingDepth").get<double>());
    }

    if (j.contains("minLoadHandlingTilt"))
    {
        msg.min_load_handling_tilt.push_back(j.at("minLoadHandlingTilt").get<double>());
    }

    if (j.contains("maxLoadHandlingTilt"))
    {
        msg.max_load_handling_tilt.push_back(j.at("maxLoadHandlingTilt").get<double>());
    }
    ...
    {
        msg.agv_acceleration_limit.push_back(j.at("agvAccelerationLimit").get<double>());
    }

    if (j.contains("agvDecelerationLimit"))
    {
        msg.agv_deceleration_limit.push_back(j.at("agvDecelerationLimit").get<double>());
    }
...
void from_json(const nlohmann::json& j, MaxArrayLens& msg)
{
    ...
    msg.actions_actions_parameters = j.at("actionsActionsParameters").get<uint32_t>();
    msg.instant_actions = j.at("instantActions").get<uint32_t>();
    msg.trajectory_knot_vector = j.at("trajectoryKnotVector").get<uint32_t>();
    msg.trajectory_control_points = j.at("trajectoryControlPoints").get<uint32_t>();
    ...
    msg.information_info_references = j.at("informationInfoReferences").get<uint32_t>();
}
...
void to_json(nlohmann::json& j, const OptionalParameters& msg)
{
    ...
    {
        throw std::runtime_error("Serialization error: Unexpected support in optional_parameters field");
    }
...
{
    msg.optional_parameters = j.at("optionalParameters").get<std::vector<OptionalParameters>>();
...
void from_json(const nlohmann::json& j, Timing& msg)
{
    ...
    {
        msg.default_state_interval.push_back(j.at("defaultStateInterval").get<double>());
    }

    if (j.contains("visualizationInterval"))
    {
        msg.visualization_interval.push_back(j.at("visualizationInterval").get<double>());
    }
}
...
void to_json(nlohmann::json& j, const TypeSpecification& msg)
{
    ...
    if (msg.agv_kinematic == "DIFF" || msg.agv_kinematic == "OMNI" || msg.agv_kinematic == "THREEWHEEL")
    {
        j["agvKInematic"] = msg.agv_kinematic;
    }
    else
    {
        throw std::runtime_error("Serialization error: Unexpected agv_kinematic in type_specification field");
    }

    if (msg.agv_class == "FORKLIFT" || msg.agv_class =="CONVEYOR" || msg.agv_class == "TUGGER" || msg.agv_class == "CARRIER")
    {
        j["agvClass"] = msg.agv_class;
    }
    else
    {
        throw std::runtime_error("Serialization error: Unexpected agv_class in type_specification field");
    }
...
void from_json(const nlohmann::json& j, TypeSpecification& msg)
{
    ...
    if (agv_kinematic == "DIFF" || agv_kinematic == "OMNI" || agv_kinematic == "THREEWHEEL")
    {
    ...
    if (agv_class == "FORKLIFT" || agv_class =="CONVEYOR" || agv_class == "TUGGER" || agv_class == "CARRIER")
    {
    ...
    {
        msg.series_description.push_back(j.at("seriesDescription").get<std::string>());
    }
}
...
void from_json(const nlohmann::json& j, Factsheet& msg)
{
    ...
    msg.physical_parameters = j.at("physicalParameters").get<PhysicalParameters>();
